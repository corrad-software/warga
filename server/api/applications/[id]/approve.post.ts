import { authenticateUser } from '~/lib/middleware/auth'
import { createAuditLog } from '~/lib/utils/audit'
import { approveApplication } from '~/lib/workflow/service'
import type { UserRole } from '@prisma/client'

export default defineEventHandler(async (event) => {
  try {
    const currentUser = await authenticateUser(event)
    const applicationId = getRouterParam(event, 'id')
    const body = await readBody(event)

    if (!applicationId) {
      throw createError({
        statusCode: 400,
        message: 'Application ID is required'
      })
    }

    // Only consular officers and admins can approve
    const allowedRoles: UserRole[] = ['PEGAWAI_KONSUL', 'ADMIN']
    if (!allowedRoles.includes(currentUser.role)) {
      throw createError({
        statusCode: 403,
        message: 'Only consular officers can approve applications'
      })
    }

    const { decision, decisionReason } = body

    // Execute approval
    const result = await approveApplication(
      applicationId,
      currentUser.id,
      currentUser.role,
      decision,
      decisionReason
    )

    if (!result.success) {
      throw createError({
        statusCode: 400,
        message: result.error || 'Approval failed'
      })
    }

    // Create audit log
    await createAuditLog({
      userId: currentUser.id,
      action: 'APPROVE',
      entityType: 'Application',
      entityId: applicationId,
      metadata: {
        decision,
        decisionReason,
        newStatus: 'APPROVED'
      }
    })

    return {
      success: true,
      data: {
        application: result.application,
        message: 'Application approved successfully'
      }
    }
  } catch (error: any) {
    console.error('Approval error:', error)

    if (error.statusCode) {
      throw error
    }

    throw createError({
      statusCode: 500,
      message: error.message || 'Failed to approve application'
    })
  }
})
